...
// (see fledge.toml). Validates every specs/*.3md structurally — parses,
// no dependency cycles, no duplicate skill names, placeholders match
// declared inputs — without executing anything. Deliberately does NOT run
// the real six targets: those need Safari, an iOS simulator, a live
// emulator, and real network access, none of which exist on a plain
...
const files = Array.from(glob.scanSync({ cwd: "specs" })).sort();
